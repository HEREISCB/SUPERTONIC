...
import { VoiceName, GeneratedAudio, SpeakerConfig } from './types';
...
const App: React.FC = () => {
  ...
  const handleSynthesize = async () => {
    ...
    try {
      let result;
      if (mode === 'single') {
        result = await synthesizeSpeech(inputText, selectedVoice, isCheerfully);
        ...
        result = await synthesizeMultiSpeaker({
          ...
          speaker1Voice: multiConfig.speaker1.voice,
          speaker2Name: multiConfig.speaker2.name,
          speaker2Voice: multiConfig.speaker2.voice,
          conversation: multiConfig.conversation,
        });
        const newAudio: GeneratedAudio = {
          id: Math.random().toString(36).substring(7),
          text: `Conversation: ${multiConfig.speaker1.name} & ${multiConfig.speaker2.name}`,
          voice: multiConfig.speaker1.voice, // Primary
          blobUrl: result.blobUrl,
          timestamp: Date.now(),
          isMultiSpeaker: true,
        };
      ...
      console.error(err);
      setError(err.message || "Failed to synthesize speech. Please check your API key.");
    } finally {
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[10px] text-zinc-500 font-bold tracking-[0.2em] uppercase">Voice Studio 2.0</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {VOICES.map((v) => (
                <VoiceCard 
                  ...
                  onSelect={(id) => {
                    if (mode === 'single') setSelectedVoice(id as VoiceName);
                  }}
                />
              ))}
            </div>
          ...
          <section className="bg-zinc-900/40 p-5 rounded-2xl border border-zinc-800">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-mono text-emerald-400">WAV (PCM)</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-400">Sample Rate</span>
                <span className="text-xs font-mono text-zinc-500">24kHz</span>
              </div>
            </div>
          </section>
        </div>
            ...
            <div className="p-6">
              {mode === 'single' ? (
                <textarea
                  ...
                  placeholder="Enter text to synthesize... Try something like 'Hello! Welcome to the Supertonic Voice Studio. How can I help you today?'"
      ...
      <footer className="border-t border-zinc-900 bg-zinc-950 py-8 px-4 mt-auto">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-zinc-600 text-xs">
            © 2025 Supertonic Voice Studio. Experimental Gemini TTS Interface.
          </p>
